...
Soft_I2C_Init(100000); //Inicio del bus I2C.
delay_ms(50); //Retardo.
...
Soft_I2C_Wr(VAL[5]);
Soft_I2C_Wr(VAL[6]);
Soft_I2C_Wr(0x10); //Se activa la salida oscilante 1Hz.
Soft_I2C_Stop();
delay_ms(50); //Retardo.
}
...
{
DS1307SetDir(0);
Soft_I2C_Wr( ShortToBcd(h.Seg) );
Soft_I2C_Wr( ShortToBcd(h.Min) );
Soft_I2C_Wr( ShortToBcd(h.Hor) );
Soft_I2C_Stop();
}
...
{
DS1307SetDir(3);
Soft_I2C_Wr( ShortToBcd(f.Dia) );
Soft_I2C_Wr( ShortToBcd(f.Fec) );
Soft_I2C_Wr( ShortToBcd(f.Mes) );
Soft_I2C_Wr( ShortToBcd(f.Ano) );
Soft_I2C_Stop();
}
